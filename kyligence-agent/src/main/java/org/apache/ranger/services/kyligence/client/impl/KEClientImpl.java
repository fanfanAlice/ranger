...
public class KEClientImpl implements IClient {
    ...
    public List<ProjectPermission> getProjectPermission(String project, String userOrGroupName) throws Exception {
        ...
        JsonObject data = new JsonParser().parse(send(url, "GET", null, true)).getAsJsonObject();
        if (null == data)
            throw new RuntimeException("Failed to get Kyligence projects by api: " + url);
    ...
    public void grantProjectPermission(String project, String type, String permission, String[] names) throws Exception {
        ...
        JsonObject data = new JsonParser().parse(send(url, "POST", s, true)).getAsJsonObject();
        if ("000".equals(data.get("code").getAsString())) {
            LOG.info("grantProjectPermission success ! project = " + project + ", names = " + names);
        }
    }
    ...
    private String send(String urlStr, String method, String data) throws IOException {
        return send(urlStr, method, data, false);
    }

    private String send(String urlStr, String method, String data, boolean accept) throws IOException {
        ...
        try {
            ...
            if (!accept) {
                connect.setRequestProperty("Accept", "application/vnd.apache.kylin-v2+json");
            } else {
                connect.setRequestProperty("Accept", "application/vnd.apache.kylin-v4-public+json");
            }
